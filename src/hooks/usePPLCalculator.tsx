...
export const usePPLCalculator = ({
  ...
  const getDryMatterBiomass = (): PPlCalculationsReturn => {
    ...
    if (
      constants.PRODUCT_DRY_MATTER_FACTOR < 0 ||
      !constants.PRODUCT_DRY_MATTER_FACTOR
    ) {
      return {
  ...
  const getResidueDryMatterBiomass = (): PPlCalculationsReturn => {
    ...
    if (
      ...
      !constants.RESIDUE_DRY_MATTER_FACTOR
    )
      return {
  ...
  const getWeedBiomass = (): PPlCalculationsReturn => {
    const totalAerialBiomass = getTotalAerialBiomass();
    const formula = `${totalAerialBiomass.name} * ${typeTranslation["WEED_BELOWGROUND_INDEX"]}`;

    if(totalAerialBiomass.canNotCalculate || constants.WEED_BELOWGROUND_INDEX < 0 || !constants.WEED_BELOWGROUND_INDEX){
      return {
          name: "Biomassa das raízes adventícias",
          result: -1,
          unity: "(t/ha)",
          formula,
          calculation: "",
          canNotCalculate: true,
          cantCalculateMessage: `${totalAerialBiomass.name} não calculada ou ${constants.WEED_BELOWGROUND_INDEX} inválido!`,
      };
    }

    const result = totalAerialBiomass.result * constants.WEED_BELOWGROUND_INDEX;
    const calculation = `${totalAerialBiomass.result} * ${constants.WEED_BELOWGROUND_INDEX}`
  ...
  const getAerialWeedBiomass = (): PPlCalculationsReturn => {
    const dryMatterBiomass = getDryMatterBiomass();
    const formula = `${typeTranslation["WEED_AERIAL_FACTOR"]} * ${dryMatterBiomass.name}`;

    if(dryMatterBiomass.canNotCalculate || constants.WEED_AERIAL_FACTOR < 0 || !constants.WEED_AERIAL_FACTOR){
      return {
          ...
          calculation: "",
          canNotCalculate: true,
          cantCalculateMessage: `${dryMatterBiomass.name} não calculada ou ${constants.WEED_AERIAL_FACTOR} inválido!`,
      };
    }

    const result = dryMatterBiomass.result * constants.WEED_AERIAL_FACTOR;
    const calculation = `${dryMatterBiomass.result} * ${constants.WEED_AERIAL_FACTOR}`
