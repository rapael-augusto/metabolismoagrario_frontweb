...
export default function ListConstantsHeader() {
	const { filterCriteria, updateFilter, filteredReferences } =
	...
	// só deve abrir o modal quando não tiver selecionado um filtro
	const openModalFirst =
		Object.values(filterCriteria).filter((value) => value != "").length === 0 &&
		filteredReferences.length !== 0;
	const [isModalOpen, setIsModalOpen] = useState(openModalFirst);
